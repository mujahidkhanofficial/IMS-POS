...
electron_1.app.whenReady().then(() => {
    ...
    const machineId = license_1.licenseManager.getMachineId();
    let isValid = false;
    if (stored?.key) {
...
electron_1.ipcMain.handle('license:getStatus', () => {
    ...
    return {
        licensed: isValid,
        machineId: license_1.licenseManager.getMachineId()
    };
});
// App Lifecycle
...
electron_1.app.on('activate', () => {
    if (electron_1.BrowserWindow.getAllWindows().length === 0) {
        // Re-check license on activate? Or just open main if licensed?
        // For simplicity, just run the startup logic again or check variable
        const stored = license_1.licenseManager.loadLicense();
